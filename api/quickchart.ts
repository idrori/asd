/**
 * Vercel Serverless Function: Generate PNG Figures via QuickChart.io
 *
 * Creates publication-quality PNG figures from chart data
 * Returns base64-encoded PNGs directly (no Blob storage needed)
 *
 * Endpoint: POST /api/quickchart
 */
  ...
  switch (chart.type) {
    case 'histogram': {
      ...
      return {
        config: {
          ...
          options: {
            ...
            plugins: {
              title: {
                display: true,
                text: `Distribution of ${safeColumnName}${chart.stats ? ` (n=${chart.stats.count}, mean=${chart.stats.mean?.toFixed(2)}, SD=${chart.stats.std?.toFixed(2)})` : ''}`,
                font: { size: 14 }
              },
        ...
        description: `Distribution of ${safeColumnName}${chart.stats ? ` (n=${chart.stats.count}, mean=${chart.stats.mean?.toFixed(2)}, SD=${chart.stats.std?.toFixed(2)})` : ''}`
      };
    }
    ...
    case 'heatmap': {
      // For heatmap, create a simplified correlation bar chart
      const corrMatrix = chart.data as Record<string, Record<string, number | null>>;
      const cols = chart.columns || Object.keys(corrMatrix);

      // Get first row correlations
      const firstCol = cols[0];
      const correlations = cols.map(col => corrMatrix[firstCol]?.[col] ?? 0);
      ...
      return {
        config: {
          ...
          data: {
            ...
            datasets: [{
              label: 'Correlation',
              data: correlations,
              backgroundColor: correlations.map(v => v >= 0 ? 'rgba(59, 130, 246, 0.7)' : 'rgba(239, 68, 68, 0.7)')
            }]
          },
          options: {
            responsive: true,
            plugins: {
              title: {
                display: true,
                text: `Correlation Matrix (${cols.length} variables)`,
                font: { size: 14 }
              }
            },
            scales: {
        ...
        description: `Correlation matrix for ${cols.length} numeric variables`
      };
    }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const { chartData }: GenerateFiguresRequest = req.body;
    ...
    console.log(`[QuickChart] Processing ${chartData.length} charts`);
    ...
    for (let i = 0; i < chartData.length; i++) {
      ...
      try {
        ...
        if (!chartResponse.ok) {
          console.error(`[QuickChart] Error for chart ${i}:`, await chartResponse.text());
          continue;
        }
        ...
        console.log(`[QuickChart] Generated ${filename} (${Math.round(base64.length/1024)}KB)`);
      } catch (chartError) {
        console.error(`[QuickChart] Error generating chart ${i}:`, chartError);
        // Continue with other charts
      }
    }

    console.log(`[QuickChart] Generated ${generatedFigures.length}/${chartData.length} figures`);
