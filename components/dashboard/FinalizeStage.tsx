...
const FinalizeStage: React.FC<FinalizeStageProps> = ({
  ...
  // Extract bibliography from tex and convert to .bib format
  const extractBibliography = (texContent: string): { texWithoutBib: string; bibContent: string; bibFilename: string } => {
    const prefix = filePrefix ? `${filePrefix}_` : '';
    const bibFilename = `${prefix}references.bib`;

    // Match thebibliography environment
    const bibMatch = texContent.match(/\\begin\{thebibliography\}\{[^}]*\}([\s\S]*?)\\end\{thebibliography\}/);

    if (!bibMatch) {
      return { texWithoutBib: texContent, bibContent: '', bibFilename };
    }

    const bibEntries = bibMatch[1];

    // Convert \bibitem entries to BibTeX format
    const bibitemRegex = /\\bibitem\{([^}]+)\}\s*([\s\S]*?)(?=\\bibitem|$)/g;
    let bibContent = '% Bibliography generated by ICIScopilot\n\n';
    let match;
    let entryCount = 0;

    while ((match = bibitemRegex.exec(bibEntries)) !== null) {
      const key = match[1].trim();
      const content = match[2].trim();

      // Try to parse author, title, year from the content
      // Format is typically: Author(s) (Year). Title. Venue.
      const yearMatch = content.match(/\((\d{4})\)/);
      const year = yearMatch ? yearMatch[1] : '2024';

      // Split by (year) to get author before and rest after
      const parts = content.split(/\(\d{4}\)\.?\s*/);
      const author = parts[0]?.trim().replace(/\.$/, '') || 'Unknown Author';
      const rest = parts[1] || '';

      // Try to extract title (usually ends with period before venue)
      const titleMatch = rest.match(/^([^.]+)\./);
      const title = titleMatch ? titleMatch[1].trim() : rest.split('.')[0]?.trim() || 'Unknown Title';

      // Rest is venue/journal
      const venue = rest.substring(title.length + 1).trim().replace(/^\.\s*/, '');

      bibContent += `@article{${key},\n`;
      bibContent += `  author = {${author}},\n`;
      bibContent += `  title = {${title}},\n`;
      bibContent += `  year = {${year}},\n`;
      if (venue) {
        bibContent += `  journal = {${venue.replace(/\.$/, '')}},\n`;
      }
      bibContent += `}\n\n`;
      entryCount++;
    }

    // If no bibitems found, keep original content as comment
    if (entryCount === 0) {
      bibContent += '% Original bibliography content:\n% ' + bibEntries.replace(/\n/g, '\n% ');
    }

    // Modify tex to use biblatex/bibtex
    let texWithoutBib = texContent.replace(
      /\\begin\{thebibliography\}\{[^}]*\}[\s\S]*?\\end\{thebibliography\}/,
      `\\bibliographystyle{plain}\n\\bibliography{${bibFilename.replace('.bib', '')}}`
    );

    return { texWithoutBib, bibContent, bibFilename };
  };
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      // 2. Extract bibliography and add .tex and .bib files
      const { texWithoutBib, bibContent, bibFilename } = extractBibliography(paper);
      const texFilename = `${prefix}icis_paper_v${currentPaperVersion}_FINAL.tex`;

      // Add .tex file (with bibliography reference instead of inline)
      zip.file(texFilename, bibContent ? texWithoutBib : paper);
      filesList.push(texFilename);
      console.log(`[SaveAll] Added TeX: ${texFilename}`);

      // Add .bib file if bibliography was extracted
      if (bibContent) {
        zip.file(bibFilename, bibContent);
      ...
      // 4. Add data file (CSV) if available
      ...
      // 5. Add feedback and oversight JSON files
      if (feedback) {
