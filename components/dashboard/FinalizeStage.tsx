...
import { saveFinalSubmission, compileLaTeX, downloadCompiledPdf, createViewerLink, openPaperViewer, getLastCompiledPdfBlob, downloadAllFigures, getCurrentSessionFigures, getPngFiguresForCompilation, getBibliographyContent, getChartDataContent, deleteCloudDataFile, getDataTableCsv, CompileResult, ViewerLinkResult } from '../../services/fileApi';
...
interface FinalizeStageProps {
  ...
  dataFileName?: string;  // Name of uploaded data file
  dataFileContent?: string;  // Content of uploaded data file
}
...
const FinalizeStage: React.FC<FinalizeStageProps> = ({
  ...
  const handleSaveAll = async () => {
    ...
    if (!paper) {
      setSaveError('Missing paper file to save');
      setIsSaving(false);
      return;
    }
    ...
    try {
      ...
      if (pdfData) {
        ...
        filesList.push(pdfData.filename);
        console.log(`[SaveAll] Added PDF: ${pdfData.filename}`);
      }

      // 2. Add .tex file
      const texFilename = `${prefix}icis_paper_v${currentPaperVersion}_FINAL.tex`;
      zip.file(texFilename, paper);
      filesList.push(texFilename);
      console.log(`[SaveAll] Added TeX: ${texFilename}`);

      // 3. Add .bib file from stored bibliography (generated during paper assembly)
      const bibData = getBibliographyContent();
      if (bibData) {
        const bibFilename = `${prefix}${bibData.filename}`;
        zip.file(bibFilename, bibData.content);
        filesList.push(bibFilename);
        console.log(`[SaveAll] Added BibTeX: ${bibFilename}`);
      }

      // 4. Add PNG figures
      const sessionFigures = getCurrentSessionFigures();
      if (sessionFigures.length > 0) {
        console.log(`[SaveAll] Adding ${sessionFigures.length} PNG figures...`);
        const figureBlobs = await downloadAllFigures();

        for (const fig of figureBlobs) {
          zip.file(fig.filename, fig.blob);
          filesList.push(fig.filename);
        }
        console.log(`[SaveAll] Added ${figureBlobs.length} PNG figures`);
      }

      // 5. Add data file (CSV) if available
      if (dataFileName && dataFileContent) {
        zip.file(dataFileName, dataFileContent);
        filesList.push(dataFileName);
        console.log(`[SaveAll] Added data file: ${dataFileName}`);
      }

      // 6. Add feedback and oversight JSON files
      if (feedback) {
        const feedbackFilename = `${prefix}feedback_v${currentRound}.json`;
        zip.file(feedbackFilename, JSON.stringify(feedback, null, 2));
        filesList.push(feedbackFilename);
      }

      if (allOversight && allOversight.length > 0) {
        for (const oversight of allOversight) {
          const oversightFilename = `${prefix}oversight_v${(oversight as any).version || 1}.json`;
      ...
      // NOTE: Conceptual figures (research_model.png, theoretical_framework.png) are now included
      // with all other figures in step 4 above via getCurrentSessionFigures()
      // The separate infographic step has been removed.

      // 8. Add data table CSV if available
      ...
      const zipFilename = `${prefix}icis_submission_v${currentPaperVersion}.zip`;
      ...
      setSaveError(error instanceof Error ? error.message : 'Failed to create zip file');
    }
    ...
    <div className="flex flex-col items-center justify-center h-full text-center max-w-lg mx-auto">
      ...
      <div className="flex flex-col gap-4 w-full">
        ...
        >
          ...
          {isSaving ? 'Creating ZIP...' : 'Download All Files (.zip)'}
        </button>
        ...
        {savedFiles.length > 0 && (
          <div className="text-sm text-emerald-600 text-center">
            <p className="font-medium mb-1">Downloaded: {savedFiles[0]}</p>
            <p className="text-xs">Contains {savedFiles.length - 1} files: {savedFiles.slice(1).map(f => f.split(/[/\\]/).pop()).join(', ')}</p>
          </div>
        )}
