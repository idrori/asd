...
`;
  ...
  for (const key of sectionOrder) {
    if (sections[key]) {
      ...
      if (key === 'title') {
      ...
      } else if (key === 'theory' && isPartialPaper && figures.length > 0) {
        // For theoretical papers, insert conceptual figures (research model, framework) in the Theory section
        ...
        if (conceptualFigures.length > 0) {
          paper += `\\subsection*{Visual Framework}\n\n`;
          paper += `The following figures provide visual representations of the theoretical framework and research model presented in this paper.\n\n`;
