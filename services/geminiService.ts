...
function fixLatexEscaping(content: string): string {
  ...
  // FIRST: Convert markdown-style formatting to LaTeX BEFORE other escaping
  // Handle **bold** -> \textbf{bold}
  fixed = fixed.replace(/\*\*([^*]+)\*\*/g, '\\textbf{$1}');

  // Handle *italic* -> \textit{italic} (but not ** which is bold)
  // This pattern matches single asterisks that aren't part of ** pairs
  fixed = fixed.replace(/(?<!\*)\*([^*\n]+)\*(?!\*)/g, '\\textit{$1}');

  // Clean up malformed $\ast$ patterns that might have been created previously
  // Pattern like: $\ast$text$\ast$ should become \textit{text}
  fixed = fixed.replace(/\$\\ast\$([^$]+)\$\\ast\$/g, '\\textit{$1}');

  // Fix backticks (markdown code) -> \texttt{code}
  fixed = fixed.replace(/`([^`]+)`/g, '\\texttt{$1}');
  ...
  // Fix asterisks used as bullets at start of line
  // Pattern: * at start of line that's not \item
 ...
 * The AI sometimes generates a "References:", "BIBLIOGRAPHY", or thebibliography
 * section within the body of a section. This strips it out since we handle
 * references separately via BibTeX.
 */
function stripEmbeddedReferences(content: string): string {
  ...
  // Pattern 4: "BIBLIOGRAPHY" (all caps) followed by content until next section or end
  // The LLM sometimes generates this heading with a thebibliography block
  cleaned = cleaned.replace(/\n*BIBLIOGRAPHY\s*\n([\s\S]*?)(?=\\section|$)/gi, '');

  // Pattern 5: \begin{thebibliography}...\end{thebibliography} blocks embedded in body sections
  cleaned = cleaned.replace(/\\begin\{thebibliography\}[\s\S]*?\\end\{thebibliography\}/gi, '');

  // Pattern 6: Standalone \bibitem{} entries that appear outside proper bibliography
  cleaned = cleaned.replace(/\\bibitem\{[^}]*\}[^\n]*\n?/gi, '');
...
/**
 * Remove section headers from LLM-generated content.
 * The LLM sometimes generates \section{} commands within its output,
 * but assemblePaper() adds its own section headers. This prevents duplicates.
 */
function stripSectionHeaders(content: string): string {
  // Remove \section{...} and \section*{...} at the start of content
  // Also handles cases where the section header might have whitespace before it
  let cleaned = content.replace(/^\s*\\section\*?\{[^}]*\}\s*/i, '');

  // Also remove any section headers that appear right after common LaTeX commands
  // that might precede section content (like after abstract end)
  cleaned = cleaned.replace(/\\section\*?\{[^}]*\}\s*\n\s*\\section\*?\{/gi, '\\section{');
...
`;
  ...
  for (const key of sectionOrder) {
    if (sections[key]) {
      ...
      // Strip any section headers that the LLM might have included in the content
      // (we add our own section headers below, so this prevents duplicates)
      if (key !== 'title' && key !== 'abstract' && key !== 'references') {
        fixedContent = stripSectionHeaders(fixedContent);
      }
...
Generate a professional research model diagram that accurately represents the constructs and relationships described in this specific research paper.`;
}
...
Generate a professional theoretical framework diagram that accurately represents the theories and concepts described in this specific research paper. This figure should be distinct from the research model diagram.`;
}
